...
public class AINativeIAMOperations<T extends IAMContext> implements AIAMOperations<T> {
    ...
    public <R extends AIResponse> Mono<R> execute(AIRequest<T> request, Class<R> responseType) {
        return Mono.fromCallable(() -> {
            ...
            // 🔥 수정: 구체적인 IAM 응답 타입 결정 (JSON 파싱 오류 해결)
            Class<? extends IAMResponse> iamResponseType = determineIAMResponseType(iamRequest);
            IAMResponse iamResponse = executeWithAudit(iamRequest, iamResponseType);
    ...
    /**
     * 🔥 DiagnosisType에 따른 구체적인 IAM 응답 타입 결정
     */
    private Class<? extends IAMResponse> determineIAMResponseType(IAMRequest<T> request) {
        if (request.getDiagnosisType() != null) {
            switch (request.getDiagnosisType()) {
                case RESOURCE_NAMING:
                    // ResourceNamingDiagnosisStrategy.ResourceNamingResponse는 구체 클래스
                    return ResourceNamingDiagnosisStrategy.ResourceNamingResponse.class;
                case CONDITION_TEMPLATE:
                case POLICY_GENERATION:
                default:
                    // 기타 진단 타입들은 기본 StringResponse 사용
                    return StringResponse.class;
            }
        }
        return StringResponse.class; // 기본값
    }
