...
public class DistributedSessionManager<T extends IAMContext> {
        ...
        try {
            ...
            for (String sessionId : inactiveSessions) {
                try {
                    ...
                    log.warn("⚠️ Failed to cleanup session: {}", sessionId, e);
    ...
    public DetailedStrategySessionInfo getStrategySessionDetails(String sessionId) {
        try {
            // 실제 구현에서는 sessionRepository를 통해 세션 정보 조회
            log.info("📊 Retrieving strategy session details for: {}", sessionId);
            
            // Mock 데이터 (실제 구현 시 Redis에서 조회)
            return DetailedStrategySessionInfo.notFound(sessionId);
            
        } catch (Exception e) {
            log.error("❌ Failed to get strategy session details for: {}", sessionId, e);
    ...
    private void saveExecutionMetrics(String sessionId, AIExecutionMetrics metrics) {
        try {
            ...
            log.warn("⚠️ Failed to save execution metrics for session: {}", sessionId, e);
        }
    }
